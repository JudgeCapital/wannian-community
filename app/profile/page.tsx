...
const ExchangeList = memo(({ exchanges }: ExchangeListProps) => {
  ...
  return (
    <div className="space-y-2">
      {/* 表头 */}
      <div className="grid grid-cols-5 items-center text-sm text-gray-500 h-14 px-6">
        <div className="text-center">交易所</div>
        <div className="text-center">UID</div>
        <div className="text-center">返佣比例</div>
      ...
      <div className="divide-y divide-gray-100">
        {exchanges.map((exchange, index) => (
          <div key={index} className="grid grid-cols-5 items-center h-14 px-6 hover:bg-gray-50">
            <div className="text-center font-mono text-gray-600">{exchange.name}</div>
            <div className="text-center font-mono text-gray-600">{exchange.uid}</div>
            <div className="text-center font-mono text-[#4285F4]">{(exchange.rate * 100).toFixed(1)}%</div>
            <div className="text-center">
              {exchange.status === 'success' ? (
                <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-sm font-mono text-green-600 bg-green-50">
                  审核成功
                </span>
              ) : exchange.status === 'pending' ? (
                <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-sm font-mono text-yellow-600 bg-yellow-50">
                  审核中
                </span>
              ) : (
                <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-sm font-mono text-red-600 bg-red-50">
                  审核失败
                </span>
              )}
            </div>
...
export default function Profile() {
  ...
  const userData = useMemo(() => ({
    ...
    boundExchanges: [
      { name: "BINANCE", uid: "78901234", rate: 0.05, status: 'success' as const },
      { name: "BITGET", uid: "56789012", rate: 0.03, status: 'pending' as const },
      { name: "OKX", uid: "34567890", rate: 0.02, status: 'failed' as const }
    ]
  }), []);
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f5]">
      ...
      <div className="fixed right-8 bottom-8 z-50">
        <button 
          className="w-14 h-14 bg-[#4285F4] hover:bg-[#2b5797] rounded-full flex items-center justify-center shadow-lg hover:shadow-xl transition-all duration-300 group"
          title="联系客服"
        >
          <svg 
